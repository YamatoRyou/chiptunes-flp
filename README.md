...
## Famicom 历史中使用过的音源芯片简易扫盲  
* Ricoh 2A03 / 2A07: 即 Famicom 自带的处理器 (2A03 为 NTSC 版本, 2A07 为 PAL 版本), 音源部分集成在处理器中. 它是本项目要模拟的主要音色.  
2 个方波 / 矩形波通道 (Pulse);  
1 个低精度的三角波通道 (Triangle);  
1 个可产生两种音色的噪波通道 (Noise);  
1 个可播放 DPCM 编码的采样通道 (DPCM)  
* Nintendo MMC5:  
具备 2 个方波 / 矩形波通道 (Pulse), 另一个 PCM 通道属于玄学问题被本项目排除.  
* Namco 163:  
具备最多 8 条通道, 每个通道最大可使用 32 * 32 精度的波形.  
* Nintendo FDS (RP2C33), 任天堂磁盘系统附带的扩展音源芯片:  
具备 1 个通道, 可使用 64 * 64 精度的波形, 支持简单的 FM.  
* Konami VRC6:  
具备 2 个方波 / 矩形波通道 (Pulse); 
1 个锯齿波通道.  
* Konami VRC7:  
具备 6 个通道, 全部使用 FM 合成方式. 该芯片属骚操作.  
* Sunsoft 5B:  
具备 3 个 方波通道, 支持将方波与噪波进行混合.  
